...
export default async function ComicPage({ params: paramsPromise }: Args) {
          ...
          <div className="flex-1 space-y-4">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex gap-2">
                ...
                <BacklogButton itemId={item.id} collectionSlug="comics" />
                <FavoriteButton itemId={item.id} collectionSlug="comics" />
              </div>
            </div>
...
const queryBySlug = cache(async ({ slug }: { slug: string }) => {
  ...
  const reviewsResult = await payload.find({
    ...
    limit: 4,
    where: {
      and: [{ 'item.relationTo': { equals: 'comics' } }, { 'item.value': { equals: item.id } }],
    },
    sort: '-createdAt',
  })
